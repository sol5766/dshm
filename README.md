...
> 技术路线：**纯内嵌运行时（embedded）**。宿主模式（Harmonybrew 桥接）已完全剥离，
> 仅保留应用自带 `libnode.so.137` + 内置 DSH 运行环境，以 `--jitless` 在鸿蒙沙箱内运行 dsh。
...
- **内嵌运行**：应用自带 `libnode.so.137` + 内置 DSH 运行环境，`--jitless` 运行，不依赖外部 brew；
  启动时自行解压运行环境至沙箱 `files/dsh` 并拉起 dsh 于本机 3080；已剥离宿主（Harmonybrew bridge）模式。
- **插件市场**：内置 dshmarket（当前 1.45.1），可直接搜索/安装/卸载第三方插件 ——
  安装走 dsh 自带的**同进程 pnpm**（绕过鸿蒙"无法 spawn 独立进程"的限制），装完即时热加载；
  国内网络下请把 dsh 设置里「下载区域」设为**中国大陆**（走腾讯 npm 镜像 / proxy 加速）。
- **终端**：右侧边栏 pty 终端（zsh/bash），支持 **Tab 补全**、历史、Ctrl-C；
  触摸场景另有 `Tab`/`↑`/`^C` 按钮。
- **运行环境面板**：逐项展示内嵌 node / deepseek-harness / 全盘访问权限状态。
...
| 项 | 内嵌模式 |
|---|---|
| 运行时 | 应用自带 `libnode.so.137`（dlopen + `node::Start`，`--jitless`）+ 内置 DSH 运行环境 |
| 会话库（DSH_HOME） | `<filesDir>/home/.dsh` |
| JIT | 无（沙箱 W^X 限制，靠 `_fetch-shim.cjs` 垫 Web/网络全局） |
| 适用 | 标准鸿蒙 PC / 2in1，零外部依赖（已剥离宿主模式） |

当前生效模式固定为 `embedded`，写入 `<filesDir>/runtime-mode-active.txt`。
...
│   ├── src/main/cpp/              # dsh_host（内嵌运行器：dlopen libnode + 环境准备）、pty addon、node shim
...
| 文档 | 内容 |
|---|---|
| [`docs/pitfalls-and-gotchas.md`](docs/pitfalls-and-gotchas.md) | **踩坑点总表**（沙箱/native、环境树、内嵌运行、profile 插件体系、pnpm、终端、PC 系统集成、构建打包、ArkTS）—— 移植或排障先看这份 |
...
- **停机用标记文件确认**（`node-exited`），且内嵌停机由 native 父子进程接管执行。
