---
title: "受限开放权限-应用权限列表-应用权限管控-程序访问控制-安全-系统"
source: "https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/restricted-permissions"
author:
published:
created: 2026-06-25
description: "受限开放权限                 申请方式     当前仅少量符合特殊场景的应用可在通过审批后，使用受限权限，其申请方式请参考：申请使用受限权限。                  ……欲了解更多信息欢迎访问华为HarmonyOS开发者官网"
tags:
  - "clippings"
---
## 申请方式
...
如果应用涉及获取受限权限，在应用发布上架时，AGC，AppGallery Connect（AGC）将根据应用的使用场景审核是否可以使用对应的受限权限。如不符合，应用的上架申请将被驳回，审核方式请见 [发布HarmonyOS应用](https://developer.huawei.com/consumer/cn/doc/app/agc-help-release-app-0000002271695230) 。
...
**其他场景下的使用方案：** 使用“联系人Picker”访问联系人数据，使用方式请参考： [使用Picker选择联系人](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/contacts-intro#%E4%BD%BF%E7%94%A8picker%E9%80%89%E6%8B%A9%E8%81%94%E7%B3%BB%E4%BA%BA) 。
...
**其他场景下的使用方案：** 使用“AudioPicker”访问用户音频文件，使用方式请参考： [选择音频类文件（FilePicker）](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/select-user-file#%E9%80%89%E6%8B%A9%E9%9F%B3%E9%A2%91%E7%B1%BB%E6%96%87%E4%BB%B6) 。
...
**其他场景下的使用方案：** 使用“AudioPicker”保存用户音频文件，使用方式请参考： [保存音频类文件（FilePicker）](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/save-user-file#%E4%BF%9D%E5%AD%98%E9%9F%B3%E9%A2%91%E7%B1%BB%E6%96%87%E4%BB%B6) 。
...
1. 应用需要批量读取本地媒体库的文件（图片、视频），并同步到云端服务器。
2. 应用需要批量读取本地媒体库的文件（图片、视频），在设备间进行同步或者克隆操作。
...
1. 应用读取自身写入到图库的图片/视频文件时，无需使用读取权限。
2. 用户在应用内手动选择图片/视频并进行处理，如：
	1）用户在应用内手动选择图片/视频，投屏到大屏等其他设备。
	2）用户在应用内手动选择图片/视频，发送、共享给其他应用或设备。
	3）用户在应用内手动选择图片/视频，对图片/视频进行加密、美化等操作。
...
**变更信息：** 在API 11，权限等级为system\_basic；从API 12开始，权限等级变更为normal，向普通应用开放。
...
**变更信息** ：API 11，权限等级为system\_core；从API 12开始，权限等级变更为system\_basic，向普通应用开放。从API 26.0.0开始，该权限支持在全设备上申请。
...
**变更信息** ：API 7-11，权限等级为system\_core；从API 12开始，权限等级变更为system\_basic，向普通应用开放。
...
**变更信息** ：API 8-13，权限等级为system\_core；从API 14开始，权限等级变更为system\_basic，向普通应用开放。

## ohos.permission.SET\_TELEPHONY\_ESIM\_STATE\_OPEN
...
**变更信息** ：在API 13，权限等级为normal；从API 14开始，权限等级变更为system\_basic。
...
允许系统JS引擎申请带MAP\_FORT标识的匿名可执行内存。

应用申请此权限后，系统引擎可申请带MAP\_FORT的匿名可执行内存，做即时编译，提高与形式执行效率。
...
## ohos.permission.kernel.ALLOW\_USE\_JITFORT\_INTERFACE

允许应用调用JITFort接口更新MAP\_FORT内存的内容。
...
**携带额外数据** ：是，配置方法请见 [Driver Development Kit开发指导](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/externaldevice-guidelines#%E5%BA%94%E7%94%A8%E7%AD%BE%E5%90%8D) 。
...
## ohos.permission.kernel.SUPPORT\_PLUGIN
...
## ohos.permission.CUSTOM\_SANDBOX
...
## ohos.permission.ACCESS\_FIDO2\_ONLINEAUTH
...
1. 应用内需具备开启闪控球功能的独立开关，开关需默认关闭，需由用户主动开启。
2. 闪控球内呈现的内容及功能需与应用内实际功能匹配。
3. 闪控球展示的内容不可用于营销、广告、引导付费等场景。
...
## ohos.permission.AUTO\_RESTORE\_MAIN\_WINDOW

允许应用使用闪控球的自动恢复到应用主窗口的能力。

**申请条件** ：需要与闪控球权限 [ohos.permission.USE\_FLOAT\_BALL](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/restricted-permissions#ohospermissionuse_float_ball) 一起，才可申请此权限。

**可申请此权限的特殊场景与功能：**

司机、外卖员、快递员职业使用职业应用进行抢单时，应用可通过闪控球，自动恢复应用主窗口并加载指定页面，帮助对应用户进行抢单。
...
## ohos.permission.READ\_WHOLE\_CALENDAR
...
## ohos.permission.WRITE\_WHOLE\_CALENDAR
...
## ohos.permission.ALLOW\_EXTERNAL\_NATIVE\_CODE
...
## ohos.permission.atomicService.MANAGE\_AGING
...
## ohos.permission.kernel.LOAD\_INDEPENDENT\_LIBRARY
...
## ohos.permission.CRYPTO\_EXTENSION\_REGISTER
...
## ohos.permission.MANAGE\_MEDIA\_RESOURCES\_FOR\_PUBLIC
...
## ohos.permission.MANAGE\_RECENT\_SNAPSHOT
...
## ohos.permission.SET\_WINDOW\_ALPHA
...
## ohos.permission.MANAGE\_CALL\_FOR\_DEVICES
...
## ohos.permission.SET\_WINDOW\_TOUCH\_AREAS
...
## ohos.permission.MANAGE\_BLUETOOTH\_ADVERTISER\_NAME
...
## ohos.permission.INPUT\_DEVICE\_CONFIGURATOR
...
在连接了多个显示屏的PC/2in1设备上，用户操作输入设备时，输入设备的驱动应用将用户的操作目标设定到具体的显示屏上。
...
## ohos.permission.REGISTER\_OBJECTEDITOR\_EXTENSION
...
仅开放给文档编辑类应用和为文档编辑应用提供特定文档片段编辑能力的应用（如公式编辑器）。
...
## ohos.permission.ALLOW\_ACCESSORY\_ACCESS
...
1. 配件发起到主机侧的文件传送时，需要使用配件接入服务接收来自配件的指令，完成主机侧应用和服务的后台唤醒。
2. 主机侧进行视频通话或者直播时，需要使用配件接入服务感知到配件在周边，并完成分布式相机服务的唤醒，以便可使用配件的摄像头。
...
## ohos.permission.CHECK\_CALL\_LOG
...
快递员职业类应用在快递员将快递放入快递柜或者让门卫代签时，需要校验快递员是否与收件客户进行过有效的通话。
...
## ohos.permission.ACCESS\_DLP\_SERVICE
...
## ohos.permission.CONTROL\_DEVICE
...
## ohos.permission.KEEP\_BACKGROUND\_RUNNING\_SPECIAL\_SCENARIO
...
## ohos.permission.GET\_CALL\_TRANSFER\_INFO
...
## ohos.permission.kernel.AS\_LDK\_DRIVER
...
## ohos.permission.PRINTER\_DRIVER
...
1. 打印驱动应用在首次打开时，新增一个特定的打印机。
2. 打印驱动应用需要一键删除所有本品牌的打印机。
