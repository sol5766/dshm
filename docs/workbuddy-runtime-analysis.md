...
## 4. 方案 A 落地记录（2026-09-11，本节为实施状态）

**已完成的代码改动**（等待签名 profile 验证）：

1. **`dsh_host.cpp` — JIT 探测 + 自动回退**：
   - 启动时先读 `<filesDir>/log/jit-capability.txt` 缓存（`jit` | `jitless`）；
   - 无缓存则跑最小探测（`-e` 脚本：`WebAssembly` 存在性 + 真 WASM 模块编译 + 3e6 次循环触发优化编译），退出码 0 = JIT 可用，否则回退 `--jitless`；
   - 结果写回缓存，之后启动零探测成本；删缓存文件即重测。
   - JIT 可用时 argv 不含 `--jitless`；`--expose-internals` 与 `-r _fetch-shim.cjs` 保持不变。
2. **`_fetch-shim.cjs` — 按 `process.execArgv` 分流**：
   - `--jitless` 时行为不变（覆盖 fetch 等全局 + WASM 桩 + Worker 预载）；
   - JIT 时**完全不覆盖**任何全局（真 WebAssembly/undici 原生 fetch 直接用），Worker 包装也跳过。
   - rawfile 副本已同步（SHA256 一致）；`ENV_VERSION=20260911-108`。
3. **签名/包名**：新调试 profile `dshmDebug.p7b` 绑定 `com.dshm.dshclient`（AGC 项目名），bundleName 由 `com.dshm.agentic` 切换过去；p7b 内嵌证书与本机 `default_dsh-OHDSH_…` p12/cer 叶子证书 SHA256 配对（`a997f2f1…`），签名链无需换。
4. **`module.json5`**：声明 `ohos.permission.kernel.ALLOW_EXECUTABLE_FORT_MEMORY` 与 `ohos.permission.ALLOW_EXTERNAL_NATIVE_CODE`（reason 字符串三语言已加）。p7b 的 `acls.allowed-acls` 已含两条权限 + `FILE_ACCESS_PERSIST`；有效期至 2026-09-18（调试 profile，7 天，到期需重新生成）。

**验证清单**（设备回来后执行）：

- [ ] 安装成功（新 bundleName + 新 profile）
- [ ] 日志出现 `=== jit probe exit=0 -> jit ===`（而非回退 jitless）
- [ ] 日志出现 `[fetch-shim] JIT mode detected: native WebAssembly/undici available`
- [ ] 内嵌模式启动到 token URL 明显变快（预期 13.6s → 6–8s）
- [ ] 对话端到端正常（`1+1` → `2`；此时走的是**原生 undici fetch**，SSE 链路需复验）
- [ ] `SIGNAL=0`、无 V8 Fatal

**已知边界**：

- profile 7 天到期后（9/18），需在 AGC 重新生成调试 profile（权限需重新带上）或走上架审核拿正式 profile；
- `com.dshm.agentic` 的旧数据（会话/凭据）不随包名迁移——用户本就无本地会话依赖，弃用即可；
- 若 AGC 后续收紧该权限的开放范围，回退路径始终存在：删 `jit-capability.txt` 缓存写入 `jitless`，或 profile 不带权限时探测自动失败。

## 5. 相关引用

- 华为官方：[JSVM-API 申请JIT权限指导](https://developer.huawei.com/consumer/cn/doc/doccenter-capabilities/jsvm-apply-jit-profile)、[应用程序包集成 bin 文件（PC/2in1）](https://developer.huawei.com/consumer/cn/doc/HarmonyOS-Guides/hap-bin)
