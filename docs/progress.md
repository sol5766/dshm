...
> 本文档记录 BrewDSH 的开发进度、已解决问题与待办事项。
> 技术路线已收敛为**纯内嵌运行时（embedded）**：宿主/Harmonybrew 桥接模式已完全剥离。
...
| 路线 | **纯内嵌运行时**：应用自带 `libnode.so.137` + 内置 DSH 环境，`--jitless` 运行；宿主（Harmonybrew bridge）模式已剥离 |
| 姊妹项目 | `dsh-OHDSH`（内置运行时），待 AGC 权限正式审批后继续开发 |

## 2026-09-14 进度（阶段：宿主剥离完成 + 内嵌可用 + 市场/工作区修复）

- **宿主模式完全剥离**：`dsh_host.cpp` 仅保留 `RunEmbeddedNode`（dlopen `libnode.so.137` + `node::Start`，`--jitless`），
  `runtime-mode-active.txt` 固定写 `embedded`；删除 host 分支/守候进程桥接。文档 `README.md` 已同步为「仅内嵌」。
- **插件市场目录超时修复（region）**：内嵌且销量 region=global 时，插件目录直连 `awesome-dsh-plugin.com`/npmjs 在设备网络 30s 超时
  （`invalid …… aborted (30S, 2 attempts)`）。由用户把 dsh 设置「下载区域」切到**中国大陆**后恢复正常（走腾讯 npm 镜像/代理）。
  根因见 `.agent-rules/bug-log.md`。
- **工作区目录 `createDirectory` invalid payload 回归修复（A+B）**：地址栏粘贴完整路径当目标/新建目录名时，
  `createDirectory` 因 name 含 `/`（`/[/\\]/` 校验）报 `invalid payload for host.createDirectory`。
  - A：确认「地址栏粘贴 → Open」天然走 `workspace.resolve`（已有目录），不触 createDirectory；
  - B：放宽 `createDirectoryRequestSchema`（lib/index.js 与 types/directory-picker.js）——去除 name 含路径分隔符的拒绝，仅拒空/`.`/`..`，允许「粘贴路径当目标」。
  - `ENV_VERSION` 20260913-120 → 121 强制重解压生效；真机验证插件、粘贴选目录均正常。
- **后续待排查**：pty 终端打开即「会话退出，code=null」（见 `.agent-rules/bug-log.md`）。
