...
const BRIDGE = `
// ==== ${MARKER} ====
// 见 scripts/patch-market-pnpm-bridge.mjs：**只在内嵌模式**启用。
// 内嵌沙箱不能 spawn 可执行文件（PATH 上没有 pnpm、filesDir 里的文件也不可 exec），
// 只能用 dsh 自带的「同进程 worker + 内置 pnpm」。
// 宿主模式不设 DSHM_INPROCESS_PNPM（native 在宿主子进程里 unsetenv）：那里能真 fork/exec，
// 必须走 dsh 自己的原生安装路径，否则会拿环境树里那份 dsh 去装插件，与 brew 的 dsh 版本
// 不一致，部分插件装不上（2026-09-13 收敛）。
...
function dshmPnpmCli() {
    if (!DSHM_INPROCESS_PNPM_ENABLED || DSHM_FILES_DIR === '')
        return '';
